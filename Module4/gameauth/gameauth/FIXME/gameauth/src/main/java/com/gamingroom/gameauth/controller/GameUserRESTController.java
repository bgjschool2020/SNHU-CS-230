...
// FIX ME: Add path annotation for gameusers 
@Path("/gameusers/")
...
public class GameUserRESTController {
    ...
    // FIX ME: Add RolesAllowed annotation for USER based on BasicAuth Security Example 
    @RolesAllowed({"USER"})
    @GET
    @Path("/{id}")
    ...
    // FIX ME: Add RolesAllowed annotation for ADMIN based on BasicAuth Security Example 
    @RolesAllowed({"ADMIN"})
    @POST
